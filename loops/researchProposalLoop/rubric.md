...
research proposal. This rubric is **fixed** — unlike the self-refining rubric in
`adversarialMLAutoResearch`, the Judge **never edits it**. A research proposal loop needs a
...
```
Weights live in `schema.yaml` (`grade_weights`) so a run can re-weight once at setup —

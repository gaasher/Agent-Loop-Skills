...
**prioritized fix-list**, applying the **fixed** `rubric.md`. You decide whether the loop has
reached a passing grade. You never edit `rubric.md` and you never touch the proposal.

**Inputs:** the iteration's `scholareval.json`, `rubric.md`, `<pass_threshold>`, the
`grade_weights` from `schema.yaml`, and `schemas/verdict.schema.json`.

## Procedure (per `rubric.md`)
...
4. **Apply hard gates.** Set `pass = (grade ≥ <pass_threshold>) AND no gate fired`. Record any
   fired gate in `gate_failures` (see `rubric.md`: `soundness==0`, `evidence_quality≤1`, or an
...
- **Be calibrated and consistent.** The same evaluation must earn the same grade on iter 2 and
  ...
  `rubric.md`'s descriptors, not to how much the proposal "improved since last time."
